...
by real (anonymized) transaction logs from the payment platform.
...
def generate_dataset(n_legit=4000, n_fraud=200):
    """Returns (X: DataFrame of features, y: ground-truth labels 0/1 for eval only)."""
...
def generate_live_feed(n=25):
    ...
    used to seed the in-memory 'database' the API serves from."""
    df = generate_dataset(n_legit=int(n * 0.85), n_fraud=int(n * 0.15) or 1).sample(n=n, random_state=RNG.integers(0, 10000)).reset_index(drop=True)
